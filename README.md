...
C++ 样本分发制品仓库，当前版本为 `0.10.1`。`maze_sample_distributor` 在本地训练中合并提供任务无关的 Learner Demand、Producer Credit、样本接入与单 consumer 内存租约池，由 Learner 镜像监管，不单独启动任务容器。它不是 Reverb 的等价实现。
